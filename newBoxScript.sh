...
main() {
    ...
    if grep -q "Open $IP:80$" RUSTSCAN.OUTPUT123 ; then f_http_enum 80; fi #HTTP
    if grep -q "Open $IP:443$" RUSTSCAN.OUTPUT123 ; then f_http_enum 443; fi #HTTP
    if grep -q "Open $IP:8080$" RUSTSCAN.OUTPUT123 ; then f_http_enum 8080; fi #HTTP
    if grep -q "Open $IP:8000$" RUSTSCAN.OUTPUT123 ; then f_http_enum 8000; fi #HTTP
...
f_smb_enum() {
    ...
    xdotool type --delay 15 "echo -e '\033[1;31mANON ACCESS NETEXEC\033[0m' && netexec smb $IP -u '' -p '' && echo -e '\033[1;31mSMB SHARES NETEXEC\033[0m' && netexec smb $IP --shares -u '' -p '' && echo -e '\033[1;31mENUM4LINUX\033[0m' && enum4linux-ng $IP -A"
    xdotool key Return
    f_rename_tab "SMB :D"
}
